...
function installOmniDownloadTrackers (re, tracker) {
    var re = new RegExp(re);
    var downloadLinks = $('a').filter(function(i) { return re.test(this.href); });
    
    downloadLinks.each(function(i) {
        this.dlParams = new Array();
        var allParams = this.search.substring(1).split("&");
        var parameter;
        for (var j = 0; parameter = allParams[j]; j++) {
            var parameterDetails = parameter.split("=");
            this.dlParams[parameterDetails[0]] = parameterDetails[1];
        }
    });

    downloadLinks.click(tracker);
}

function TrackLinks(tracker) {
    installOmniDownloadTrackers("download.*?mozilla.org|download.html", tracker);
}
...
/* Plugin Config */
s.usePlugins=true
function trackPersonasVideo(link) {
    var s = s_gi(s_account);
    s.linkTrackVars = 'events';
    s.linkTrackEvents = 'event13';

    s.events = 'event13';
    s.tl(link,'o','Personas Video Play');
}
$('a#tour-video').live('click', function() {
    trackPersonasVideo(this);
});
$('a.mozilla-video-control-overlay').live('click', function() {
    var video = $(this).parent().find('video');

    if(video.find('source').attr('src').indexOf('personas') != -1) {
        trackPersonasVideo(this);
    }
});
...
function s_doPlugins(s) {
    ...
    var re_entry3   = new RegExp('^en-US/firefox/(personal|upgrade|ie|firefox).html$');
    var link_tracker_enabled = true;
    ...
    if(!s.prop18) {
        s.prop18 = s.getQueryParam('video');
    }
    ...
    if (re_download.test(s.pageName)) {
        ...
        s.prop8  = "Update Page";
        s.events = "event8";
    } else if (re_entry.test(s.pageName) 
                || re_entry2.test(s.pageName)
                || re_entry3.test(s.pageName)
              ) {
        s.prop8  = "Entry Page";
        s.events = "event7";
        link_tracker_enabled = false;
    }
    if(link_tracker_enabled) {
        setTracker = setTrackerGen(s.prop8);
        TrackLinks(setTracker); 
    }
